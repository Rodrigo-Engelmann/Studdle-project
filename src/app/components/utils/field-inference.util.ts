...
// A ideia é que ele converte um objeto comum em uma lista de campos
// inferFields({ titulo: '', duracao: 0, publicado: false })
//  ex.: [ {key: 'titulo',  type: TEXT }, { key: 'duracao', type: NUMBER  }, ... ]
...
export function inferFields(model: Record<string, unknown>, overrides: Record<string, Partial<FormFieldConfig>> = {}): FormFieldConfig[] {
...
function inferType(key: string, value: unknown): FieldType {
  ...
  if (name.includes('senha') || name.includes('password'))
